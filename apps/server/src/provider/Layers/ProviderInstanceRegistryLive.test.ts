 ...
 *     (`codex`, `claudeAgent`, `cursor`, `grok`, `opencode`, `dsh`) in a single
 ...
 * / `dsh` binaries. That keeps the assertions focused on registry routing
...
import { DshDriver } from "../Drivers/DshDriver.ts";
...
const makeDshConfig = (overrides: Partial<DshSettings>): DshSettings => ({
  enabled: false,
  binaryPath: "dsh",
  homePath: "",
  ...overrides,
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      const dshId = ProviderInstanceId.make("dsh_default");
      ...
      const dshDriverKind = ProviderDriverKind.make("dsh");
      ...
      const { registry } = yield* makeProviderInstanceRegistry<BuiltInDriversEnv>({
        drivers: [CodexDriver, ClaudeDriver, CursorDriver, GrokDriver, OpenCodeDriver, DshDriver],
        configMap,
      });
      ...
      expect(instances.map((instance) => instance.instanceId).toSorted()).toEqual(
        [codexId, claudeId, cursorId, grokId, openCodeId, dshId].toSorted(),
      );
      ...
      const dsh = yield* registry.getInstance(dshId);
      expect(codex?.driverKind).toBe(codexDriverKind);
      expect(claude?.driverKind).toBe(claudeDriverKind);
      expect(cursor?.driverKind).toBe(cursorDriverKind);
      expect(grok?.driverKind).toBe(grokDriverKind);
      expect(openCode?.driverKind).toBe(openCodeDriverKind);
      expect(dsh?.driverKind).toBe(dshDriverKind);
