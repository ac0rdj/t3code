...
/**
 * Build a DeepSeek Harness text-generation closure bound to a specific
 * `DshSettings` payload. Each call opens a short-lived ACP session, prompts
 * once, and collects the assistant text.
 */
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        yield* applyDshAcpModelSelection({
          ...
          mapError: (cause, configId) =>
            new TextGenerationError({
              operation,
              detail: `Failed to set DeepSeek Harness config option "${configId}" for text generation.`,
              cause,
            }),
        });
          ...
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({
                  operation,
                  detail: "DeepSeek Harness request timed out.",
                }),
              ),
        ...
        Effect.mapError((cause) =>
          isTextGenerationError(cause)
            ...
            : new TextGenerationError({
                operation,
                detail: "DeepSeek Harness ACP request failed.",
                cause,
              }),
        ),
      );
        ...
        return yield* new TextGenerationError({
          ...
          detail:
            promptResult.stopReason === "cancelled"
              ...
              : "DeepSeek Harness returned empty output.",
        });
      }
        ...
        Effect.catchTags({
          SchemaError: (cause) =>
            Effect.fail(
              new TextGenerationError({
                operation,
                detail: "DeepSeek Harness returned invalid structured output.",
                cause,
              }),
            ),
        }),
      ...
      Effect.mapError((cause) =>
        isTextGenerationError(cause)
          ...
          : new TextGenerationError({
              operation,
              detail: "DeepSeek Harness text generation failed.",
              cause,
            }),
      ),
      Effect.scoped,
